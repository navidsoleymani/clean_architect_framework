...
from pyelzhen.utils.schemas import QueryParamsSchema, PaginationSchema
from .__macros import QUERY_PARAMS_KEYS
from pyelzhen.utils.schemas import ResponseSchema
...
class BaseAPIViewWrapper(ABC, APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            ret = Response(status=HTTP_401_UNAUTHORIZED)
            ret.delete_cookie('loggedToken')
            return ret
        ...
        rslt = self.view_creator_func(request, **kwargs).post(**kwargs2)
        if type(rslt) == ResponseSchema:
            return rslt.fire()
